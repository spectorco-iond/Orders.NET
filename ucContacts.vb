...
Public Class ucContacts
    ...
    Private Sub LoadGrdContacts()
        ...
        Call cContact.contactInstructions("CONTACT_INSTRUCTIONS")
        ...
        Try
            ...
            '========================================
            'Modified May 31, 2017 to add full name criteria - T. Louzon
            strSql =
            " SELECT    FullName, RTRIM(cnt_f_fax) AS cnt_f_fax, RTRIM(cnt_email) AS cnt_email, ID " &
            " FROM      cicntp WITH (Nolock) " &
            " WHERE  ISNULL(active_y,0) = 1  And cmp_wwn =  '" & strCmp_WWN & "' "
            ' " ORDER BY  FullName "

            '++ID 11.13.2019 added criteria for exclude inactive contacts active_y = 1 

            'ADD IN FULL NAME CRITERIA
            If txtFullNameFilter.Text <> "" Then
                If optStartsWith.Checked = True Then
                    strSql = strSql & " AND fullname like '" & Trim(txtFullNameFilter.Text) & "%' "
                Else
                    strSql = strSql & " AND fullname like '%" & Trim(txtFullNameFilter.Text) & "%' "
                End If
            End If
...
#Region "Add contacts section #############################################"
    ...
    Private Sub cmdInsertNewContact_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdInsertNewContact.Click
        ...
        Try
            ...
            strSql = " SELECT * FROM cicntp WITH (Nolock) WHERE ISNULL(active_y,0) = 1 And cmp_wwn = '" & m_oOrder.Ordhead.Customer.cmp_wwn & "' ORDER BY ID DESC "
